...
nvigi::Result EnglishNormalizer::normalizeUsingFar(const std::string &textPrompt, std::string &textNormalized)
{
    ...
    // Remove standalone apostrophes (not part of contractions)
    // First, temporarily protect apostrophes between letters
    std::regex protect_contractions(R"(([a-z])'([a-z]))");
    textNormalized = std::regex_replace(textNormalized, protect_contractions, "$1###APOSTROPHE###$2");
    // Remove all remaining apostrophes
    std::regex remove_apostrophes(R"(')");
    textNormalized = std::regex_replace(textNormalized, remove_apostrophes, "");
    // Restore protected apostrophes
    std::regex restore_contractions(R"(###APOSTROPHE###)");
    textNormalized = std::regex_replace(textNormalized, restore_contractions, "'");
...
std::vector<std::string> EnglishNormalizer::splitSentences(std::string &text, int minSizeChunk, int maxSizeChunk)
{
...
nvigi::Result EnglishNormalizer::g2p(const std::string &text, std::vector<std::wstring> &phones)
{
    ...
    for (std::sregex_iterator it = wordsBegin; it != wordsEnd; ++it)
    {
        ...
        if (eng_dict.find(wordUpper) != eng_dict.end())
        {
        ...
        else
        {
            ...
            {
                if (arpabet_to_ipa.find(ph) != arpabet_to_ipa.end())
                {
