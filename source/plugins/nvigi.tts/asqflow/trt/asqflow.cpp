...
#include "source/plugins/nvigi.tts/asqflow/trt/asqflow.h"
...
#include "source/plugins/nvigi.tts/asqflow/trt/versions.h"
...
{
#if USE_TRT
    return plugin::tts::asqflow_trt::kId;
#else
    return plugin::tts::asqflow_cpu::kId;
#endif
}
...
//! ONLY.
NVIGI_PLUGIN_DEFINE("nvigi.plugin.tts.asqflow-trt", Version(VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH),
...
nvigi::Result asqflowEvaluate(nvigi::InferenceExecutionContext *execCtx, bool async = false)
{
    ...
    instance->params.minSizeChunk = std::max(runtime->minChunkSize, 50);   // Minimum 50 characters
    instance->params.maxSizeChunk = std::max(runtime->maxChunkSize, instance->params.minSizeChunk);
    instance->params.seed = runtime->seed;
    // Get Transcript data
