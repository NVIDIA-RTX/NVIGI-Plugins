                                        ...
                                        cudaStream_t cudaStream)
{
    ...
    // std::mt19937 gen(rd());
    std::mt19937 gen(-725171668); // Set seed here
    ...
    try
    {
        ...
        for (int i = 0; i < nTimeSteps; i++)
        {
            ...
            CHECK(yInTensor.copyToHost(y.data()));
...
nvigi::Result ASqFlow::evaluate(const InferenceParameters &inferParams, InternalCallback callback, cudaStream_t cudaStream)
{
    ...
    std::vector<std::string> sentencesSplit = textNormalizer->splitSentences(textNormalized);
    ...
    for (const std::string &textPieceNormalized : sentencesSplit)
    {
        ...
        // We clip the dpvalue between min(280,spectogramTargetLength) and 2000.
        // We have set this shape constraint when converting model to TRT
        dpValFloat *= 1/inferParams.speed;
        dpVal = std::min((unsigned int)dpValFloat, (unsigned int)2000);
        //dpVal = std::max(spectrogramLength, dpVal);
        dpVal = std::max((unsigned int)280, dpVal);
        ...
        res = runGeneratorModel(inputNetPhonesTextToGenAndTranscript, spectrogramTarget, dpVal, outputMelSpectrogram, cudaStream);
        if (res != kResultOk)
        {
            callback(outputAudio, textPieceNormalized, InferenceState::ERROR_HAPPENED);
            return res;
        ...
        if (indexPiece == 0 && !timers.contains("Total time to process first chunk"))
        {
