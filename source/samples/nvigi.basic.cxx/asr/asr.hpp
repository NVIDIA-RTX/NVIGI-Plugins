...
public:
    ...
    static std::expected<std::unique_ptr<Instance>, Error> create(
        ...
        instance->impl_->plugin_id = config.backend == "d3d12" ? plugin::asr::ggml::d3d12::kId :
            config.backend == "vulkan" ? plugin::asr::ggml::vulkan::kId :
            plugin::asr::ggml::cuda::kId;
    ...
    public:
        ...
        std::expected<AsyncOperation, Error> send_audio_async(
            ...
            InferenceDataAudio audio(context_->cpu_data.back());
            
            context_->slots.push_back({kASRWhisperDataSlotAudio, audio});
    ...
    private:
        ...
        mutable std::mutex mutex_;                   // Protects context access
        std::shared_ptr<AsyncContext> context_;
        std::shared_ptr<std::string> response_buffer_;
