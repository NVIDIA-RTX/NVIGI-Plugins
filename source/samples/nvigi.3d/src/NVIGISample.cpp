    ...
    , m_ScriptingConfig(scriptingConfig)
{
    ...
    std::filesystem::path mediaPath = app::GetDirectoryWithExecutable();
    for (int i = 0; i < 6; i++)
    {
        mediaPath = mediaPath.parent_path();
        ...
        if (std::filesystem::exists(mediaPath / "data" / "nvigi.test"))
        {
            mediaPath.append("data").append("nvigi.test").append("nvigi.3d");
            break;
        }
        if (i == 5)
        {
